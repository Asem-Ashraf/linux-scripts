...
#    echo "${mib_int}.${mib_dec} MB"
    printf "%3d.%2.2d MB" $mib_int $mib_dec
  elif [ $bytes -gt 999 ]; then
#    echo "${kib} KB"
    printf "%3d KB" $kib
  else
#    echo "${bytes} B"
...
do
  read line
  update_rate
  #foo=$(inxi -C | grep avg | sed -E 's/^.*avg (.*) 12min.*$/\1/')
  foo=" $(cat /proc/cpuinfo | grep MHz | sed -E 's/.*: //' > file ; awk '{s+=$1} END {print int(s/16)}' file) MHz"
  echo ",[{\"full_text\":\"${rate}\" },{\"full_text\":\"${foo}\" },${line#,\[}" || exit 1
done)
